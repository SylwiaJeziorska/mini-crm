                        ...
                        <form method="POST" action="{{ route('employees.update', $employee->id) }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    <div class="form-group form-group-default">
                                        <label>Company</label>
                                        <select class="form-control" name="company_id" value="{{ $employee->companies->id }}">
                                            @foreach($companies as $company)
                                                <option value="{{ $company->id }}" {{ ($employee->companies->id== $company->id ? "selected": "") }}>{{ $company->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
